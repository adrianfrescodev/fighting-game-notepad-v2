...
export default function CharacterMaker({ addCharacter, setIsCreate }) {
  ...
  return (
    <div className="z-9999 fixed inset-0 flex w-full items-center justify-center bg-[rgba(0,0,0,0.5)]">
      <div className="bg-tile border-border z-9999 fixed grid h-[70%] w-[30%] grid-cols-2 grid-rows-8 rounded border p-4">
        <div className="col-span-2 row-span-2 row-start-3 flex flex-col items-center justify-center p-8">
          ...
          <input
            ref={inputRef}
            className="text-text border-text bg-tile w-full rounded border p-2"
            value={name}
        ...
        <div className="col-span-2 row-start-8 flex gap-8 p-4">
          ...
          <button
            className="text-text border-border bg-accent-hover flex-1 cursor-pointer rounded border-2 hover:scale-105"
            onClick={async () => {
              const added = await addCharacter(name);
              if (added) {
                setIsCreate(false);
              } else {
